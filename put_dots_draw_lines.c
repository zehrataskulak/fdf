...
/*   Updated: 2025/12/09 16:41:08 by zzehra           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_dots(t_vars *var, t_map *map, int x_len, int y_len)
{
	...
	{
		if (map[indx].iso_x >= 0 && map[indx].iso_x < SIZE_X
			&& map[indx].iso_y >= 0 && map[indx].iso_y < SIZE_Y)
		{
			dst = var->addr + (map[indx].iso_y * var->line_len + map[indx].iso_x
					* (var->bpp / 8));
...
void	draw_lines_1(t_draw_lines_vars *drw_var)
{
...
void	draw_lines_2(t_vars *var, t_map point_1, t_map point_2)
{
	char				*dst;
...
void	draw_lines(t_vars *var, t_map *map, int x_len, int y_len)
{
	...
	while (indx < x_len * y_len)
	{
		if ((indx + 1 < x_len * y_len) && ((indx + 1) % x_len != 0))
			draw_lines_2(var, map[indx], map[indx + 1]);
		if (indx + x_len < x_len * y_len)
			draw_lines_2(var, map[indx], map[indx + x_len]);
		indx++;
	}
}
